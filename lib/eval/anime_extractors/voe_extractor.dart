// Ported from Mangayomi's voe_extractor.dart (Apache-2.0). See NOTICE.
// Mangayomi's `selectFirst`/`select` (with `:contains`) become plain
// `querySelector`/`querySelectorAll` plus a manual text scan, and
// `path.dirname` becomes [urlDirname].
import 'dart:convert';
...
class VoeExtractor {
  ...
  Future<List<Video>> videosFromUrl(String url, String? prefix) async {
    ...
    try {
      ...
      final allScripts = document.querySelectorAll('script');
      final List<Element>? alternativeScript = allScripts.isEmpty
          ? null
          : allScripts
              .where((script) => scriptBase64Regex.hasMatch(script.text))
              .toList();

      // Mangayomi used `script:contains(const sources|var sources|wc0)`; the
      // html package has no `:contains`, so scan the scripts by hand.
      Element? script;
      for (final s in allScripts) {
        final t = s.text;
        if (t.contains('const sources') ||
            t.contains('var sources') ||
            t.contains('wc0')) {
          script = s;
          break;
        }
      ...
      final m3u8Host = "${uri.scheme}://${uri.host}${urlDirname(uri.path)}";
