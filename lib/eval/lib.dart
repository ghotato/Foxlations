...
ExtensionService getExtensionService(
  ...
  switch (source.sourceCodeLanguage) {
    ...
    case 'kotlin':
      // Kotlin/Tachiyomi/Aniyomi extension run in the embedded JVM. installSource
      // downloads + (for anime apks) dex→jar-converts the artifact and records the
      // absolute jar path + entry class on config; `lang` selects the sub-source
      // for multi-language SourceFactory extensions.
      return KotlinExtensionService(
        baseUrl: effectiveBaseUrl,
        jar: source.config['jar']?.toString() ?? 'extension.jar',
        lang: source.lang.isNotEmpty ? source.lang : null,
        entry: source.config['entry']?.toString(),
      );
    case 'dart':
    default:
