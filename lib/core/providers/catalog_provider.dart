...
class CatalogProvider extends ChangeNotifier {
  ...
  Future<void> loadMore() async {
    ...
    try {
      ...
      // Surface every source failure (Kotlin/JS/Dart) in the in-app error log
      // (Settings → Advanced → Debug logs), not just the "Failed to load" screen.
      await logger.error(
        'Source "${source.name}" failed to load ${_currentTab.name}',
