...
class CookieStore {
  ...
  /// Every stored clearance as `{domain: "k=v; k2=v2"}`.
  ///
  /// The Android extension host (ApkBridge's NetworkHelper) has no WebView or
  /// FlareSolverr of its own — it just carries whatever is in its cookie jar. So
  /// we hand it every Cloudflare cookie the app holds on each invoke and let it
  /// inject them into that jar. Reads both the in-memory cache and the persisted
  /// `cookies_<domain>` keys so it survives a cold start.
  Future<Map<String, String>> allCookieHeaders() async {
    final prefs = await SharedPreferences.getInstance();
    final domains = <String>{..._cache.keys};
    for (final key in prefs.getKeys()) {
      if (key.startsWith(_prefix)) domains.add(key.substring(_prefix.length));
    }
    final result = <String, String>{};
    for (final d in domains) {
      final map = <String, String>{};
      final cached = _cache[d];
      if (cached != null) map.addAll(cached);
      final stored = prefs.getStringList('$_prefix$d');
      if (stored != null) {
        for (final e in stored) {
          final idx = e.indexOf('=');
          if (idx > 0) map[e.substring(0, idx)] = e.substring(idx + 1);
        }
      }
      if (map.isNotEmpty) {
        result[d] = map.entries.map((e) => '${e.key}=${e.value}').join('; ');
      }
    }
    return result;
  }
